...
let
  ...
  inherit (lib.fileset) isFileset unions empty;
...
rec {
    ...
    }:
    {
      ...
      functor =
        if functor ? wrappedDeprecationMessage then
          functor // { wrapped = functor.wrappedDeprecationMessage { loc = null; }; }
        else
          functor;
  ...
  attrsWith =
    ...
    mkOptionType rec {
      ...
      functor = (elemTypeFunctor "attrsWith" { inherit elemType lazy placeholder; }) // {
      ...
      // {
        ...
        merge = {
          ...
          v2 =
            ...
            in
            orig // { headError = headError'; };
        };
      }
    else
      elemType // { check = x: elemType.check x && check x; };
